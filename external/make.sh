...
BUILD()
{
    ...
    for CMD in "${CMDS[@]}"; do
        ...
        while true; do
            ...
            # Gradle wrapper distribution downloads occasionally suffer transient
            # connection resets on hosted runners. Retry only those downloads.
            if [[ "$CMD" == *"./gradlew "* ]] && \
                    [[ "$OUT" == *"services.gradle.org"* || "$OUT" == *"Connection reset"* ]] && \
                    [ "$RETRY_COUNT" -lt "$MAX_RETRIES" ]; then
                RETRY_COUNT=$((RETRY_COUNT + 1))
                LOGW "Gradle wrapper download failed; retrying ($RETRY_COUNT/$MAX_RETRIES)..."
...
elif [[ "$1" == "--check-patch-test-tools" ]]; then
    if ! $APKTOOL && ! $SMALI; then
        exit 0
    else
        exit 1
    fi
elif [[ "$1" == "--patch-test-tools" ]]; then
    PATCH_TEST_TOOLS=true
elif [ "$1" ]; then
...
if ! $PATCH_TEST_TOOLS && $ANDROID_TOOLS; then
    ANDROID_TOOLS_CMDS=(
...
if ! $PATCH_TEST_TOOLS && $EROFS_UTILS; then
    EROFS_UTILS_CMDS=(
        "git reset --hard"
...
if ! $PATCH_TEST_TOOLS && $IMG2SDAT; then
    IMG2SDAT_CMDS=(
...
if ! $PATCH_TEST_TOOLS && $SAMLOADER; then
    SAMLOADER_CMDS=(
	    "git reset --hard"
...
if ! $PATCH_TEST_TOOLS && $SIGNAPK; then
    SIGNAPK_CMDS=(
        "./gradlew build"
...
if ! $PATCH_TEST_TOOLS && $OMCDECODER; then
    OMCDECODER_CMDS=(
