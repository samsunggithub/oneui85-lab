...
OPTICS_DIR="$WORK_DIR/optics"
if [ -d "$OPTICS_DIR" ]; then
    find "$OPTICS_DIR" -type f -exec sed -i "s/SAOMC_SM-S938B/SAOMC_${TARGET_MODEL}/g" {} +
fi
...
done <<< "$(find "$OPTICS_DIR" -type f -name "cscfeature.xml" 2>/dev/null)"
