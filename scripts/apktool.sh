...
BUILD()
{
    ...
    if [ -d "$OUTPUT_PATH/smali" ]; then
        ...
        while IFS= read -r d; do
            ...
            if [ "${PATCH_TEST_MODE:-false}" = "true" ] && [ "${PATCH_TEST_SKIP_UNCHANGED_DEX:-false}" = "true" ]; then
                local DEX_HASH_DIR="$OUTPUT_PATH/.patch-test-smali-hashes"
                local DEX_HASH_FILE="$DEX_HASH_DIR/$(basename "$d").sha256"
                local DEX_CURRENT_HASH_FILE="$DEX_HASH_DIR/$(basename "$d").current"
                local ORIGINAL_DEX_FILE="$DEX_HASH_DIR/dex/$DEX_FILENAME"

                if [ -f "$DEX_HASH_FILE" ]; then
                    (
                        cd "$d"
                        find . -type f -print0 | sort -z | xargs -0 sha256sum
                    ) > "$DEX_CURRENT_HASH_FILE"

                    if cmp -s "$DEX_HASH_FILE" "$DEX_CURRENT_HASH_FILE"; then
                        rm -f "$DEX_CURRENT_HASH_FILE"
                        cp -a "$ORIGINAL_DEX_FILE" "$OUTPUT_PATH/$DEX_FILENAME"
                        continue
                    fi
    ...
    if [ "${PATCH_TEST_MODE:-false}" = "true" ]; then
        LOG "- Skipping signing and zipaligning for patch fixture"
...
DECODE()
{
    ...
    if [ -f "$OUTPUT_PATH/classes.dex" ]; then
        ...
        if [ "${PATCH_TEST_MODE:-false}" = "true" ] && [ "${PATCH_TEST_SKIP_UNCHANGED_DEX:-false}" = "true" ]; then
            ...
            mkdir -p "$DEX_HASH_DIR/dex"
            find "$OUTPUT_PATH" -maxdepth 1 -type f -name "*.dex" -exec cp -a {} "$DEX_HASH_DIR/dex/" \;
        fi
        ...
        if [ "${PATCH_TEST_MODE:-false}" = "true" ] && [ "${PATCH_TEST_SKIP_UNCHANGED_DEX:-false}" = "true" ]; then
            ...
            mkdir -p "$DEX_HASH_DIR"
            while IFS= read -r DEX_DIR; do
                (
                    cd "$DEX_DIR"
                    find . -type f -print0 | sort -z | xargs -0 sha256sum
                ) > "$DEX_HASH_DIR/$(basename "$DEX_DIR").sha256"
            done < <(find "$OUTPUT_PATH" -maxdepth 1 -type d -name "smali*")
        fi
    fi
