...
if [ "${PATCH_TEST_MODE:-false}" = "true" ]; then
    DEPENDENCIES=(
        "awk" "basename" "bc" "cat" "cp" "curl" "cut" "dd" "debugfs" "dirname"
        "du" "file" "git" "grep" "head" "java" "ln" "make" "md5sum"
        "mkdir" "mv" "perl" "python3" "rm" "sed" "sha1sum" "sort" "stat"
        "tail" "tar" "touch" "tr" "unzip" "wc" "xargs" "xxd" "zip" "zstd" "7z"
    )
else
    DEPENDENCIES=(
        "awk" "basename" "bc" "brotli" "cat" "clang" "cmake"
        "cp" "curl" "cut" "dd" "debugfs" "dirname" "du" "zip" "zstd"
...
if [ "${PATCH_TEST_MODE:-false}" = "true" ]; then
    if ! "$SRC_DIR/external/make.sh" --check-patch-test-tools; then
        LOG_STEP_IN true "Building patch-test tools..."
        "$SRC_DIR/external/make.sh" --patch-test-tools || return 1
        LOG_STEP_OUT
    fi
elif ! "$SRC_DIR/external/make.sh" --check-tools; then
