...
for i in "${FIRMWARES[@]}"; do
    ...
    while true; do
        ...
        samloader -m "$MODEL" -r "$CSC" -i "$IMEI" -s "$SERIAL_NO" download$STR -O "$ODIN_DIR/${MODEL}_${CSC}" &
        SAMLOADER_PID="$!"

        # Samsung FUS downloads can remain quiet for an extended period. Keep the
        # runner informed without changing samloader arguments or retry behavior.
        while kill -0 "$SAMLOADER_PID" 2>/dev/null; do
            sleep 60
            kill -0 "$SAMLOADER_PID" 2>/dev/null && LOG "- Firmware download still in progress (attempt $COUNT)..."
        done

        wait "$SAMLOADER_PID" || exit 1
        )
